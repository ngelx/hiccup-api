...
RSpec.describe TripsController, type: :controller do
  ...
  describe 'POST create' do
    context 'Success' do
      let(:attrs) { attributes_for(:trip, name: "Trip test", start_date: DateTime.current, end_date: DateTime.current + 2, local_contact_attributes: attributes_for(:local_contact)) }
      ...
      context 'Content' do
        ...
        it {expect(r.has_key?(:intineraries)).to be true }
        it {expect(r[:intineraries].length).to eq 2 }
        it {expect(r.has_key?(:share)).to be true }
        it {expect(r[:share]).to be_truthy }
        it {expect(r.has_key?(:local_contact)).to be true }
